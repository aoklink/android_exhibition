package com.link.exhibition.framework.component.adapterdelegate;
...
public abstract class AdapterDelegate<T> {
    ...
    private List<Integer> cacheTypes = new ArrayList<>();
    ...
    /**
     * 此处返回当前类处理的Type类型，如果当前对此position未处理，则返回-1
     * @param items
     * @param position
     * @return -1标识不处理，无类型返回0
     */
    protected int getItemViewType(@NonNull T items, int position) {
        return TypeIndex.TYPE_NOLL;
    }

    boolean isForViewType(int viewType){
        return cacheTypes.contains(viewType);
    }
     ...
     */
    @NonNull
    abstract protected RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType);
